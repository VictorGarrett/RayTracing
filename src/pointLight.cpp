...
Color lighting(Material &material, float **transform, const PointLight &light,
               const Vec4 &point, const Vec4 &eyev, const Vec4 &normalv,
               const bool in_shadow) {
  Color effective_color =
      material.get_color(point, transform) * light.intensity;
...
Color shade_hit(World &world, Computations &comps, int remaining) {
  ...
  //   float red = comps.object->material.get_color().red;

  //   if (red < 0.92 && red > 0.9 && comps.normalv.y < 0 && comps.point.x <
  //   -0.3) {
  //     std::cout << '\n' << shadowed << '\t';
  //   }

  Color surface =
      lighting(comps.object->material, comps.object->inverseTransform,
               world.light, comps.point, comps.eyev, comps.normalv, shadowed);
...
Color refracted_color(World &world, Computations comps, int remaining) {
  ...
  Color color = color_at(world, refract_ray, remaining - 1) *
